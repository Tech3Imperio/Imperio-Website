...
import { navbar_txt } from "../../assets/Images/contest";
...
const MenuItems: React.FC<MenuItemsProps> = ({ handleClick, type }) => (
  <>
    {[
      ...
      // { to: "/newproducts", label: "New Products" },
    ].map((item) => (
      <MenuItem
        key={item.to}
        to={item.to}
...
export const Navbar: React.FC = () => {
  ...
  return (
    <nav
      className={`bg-black sticky top-0 z-50  border-transparent transition-all duration-300 ease-out`}
    >
          ...
          <>
            <div className="flex-grow flex justify-center">
              ...
              <a href="https://www.imperiorailing.com/contest">
                <img
                  src={navbar_txt}
                  alt="Contest"
                  className="h-50 w-50 px-4 justify-center items-center border border-transparent transition-700 hover:shadow-small"
                />
              </a>
            </div>
            <div className=" flex gap-6 justify-center items-center">
              ...
              <Link to="/dealer-login" className=" flex justify-center pt-2">
                <PiUserCircle
                  size={50}
                  className="text-white rounded-none flex items-center justify-center hover:bg-[--black] hover:text-[--secound] transition-700 cursor-pointer"
                />{" "}
              </Link>
            </div>
          </>
