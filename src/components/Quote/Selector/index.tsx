...
export const QuoteSelector: React.FC<QuoteSelectorProps> = ({
  ...
  useEffect(() => {
    ...
    if (selectedImage) {
      if (stage === 2 || stage === 3) {
        ...
        setImage(selectedImage[Object.keys(selectedImage)[1]]); // Use the first available color image
      } else {
  ...
  return (
    <div className="h-full xl:h-[78vh] laptop:h-[60vh] flex flex-col laptop:flex-row justify-between">
      <aside className="h-full">
        <img
          src={image}
          alt="product"
      ...
      <aside className="w-screen laptop:w-[40rem] -ml-4 laptop:ml-0 px-4 phone:px-8 tablet:px-12 laptop:px-16 py-10 laptop:pt-11 laptop:pb-28 flex flex-col gap-10 laptop:gap-0 justify-center laptop:justify-between">
        ...
        >
          {stagesContains[stages[stage]].map((text, index) => (
            <button
              ...
              key={index}
              className={classNames(
                "border-2 rounded-full  mt-3 py-3 -mb-5 px-6 text-sm phone:text-base phone:px-3 tablet:text-base tablet:px-6 tablet:mr-5 laptop:px-6 laptop:py-3 laptop:text-base laptop:-mt[4rem] xl:text-base xl:mx-4 transition-500 hover:border-[--secound]",
                {
                  "bg-[--black] text-[--secound] border-transparent":
                    data[stages[stage]] === text,
                  "bg-transparent text-black border-black":
        ...
        >
          <button
            type="button"
            onClick={() => setContact((prev) => !prev)} // Handle contact form display
            className="py-2 laptop:py-4 px-3 laptop:px-6 text-base tablet:mr-16 laptop:text-lg font-semibold text-white bg-[--black] hover:text-[--black] hover:bg-[--secound] transition-500 rounded-full flex gap-2"
          >
            Proceed Further
            <FaArrowRight className="text-2xl laptop:text-3xl" />
          </button>
