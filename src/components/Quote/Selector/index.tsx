...
export const QuoteSelector: React.FC<QuoteSelectorProps> = ({
  ...
  const [stages, stage, setStage] = stagesprops;
  const [data, setData] = dataprops;
  const [colors, setColor] = colorsprops;
  ...
  const currentStageData = useMemo(() => {
    ...
    return fetchData[stages[stage]];
  }, [fetchData, stages, stage]);

  useEffect(() => {
    if (!currentStageData) return;

    const selectedData = data[stages[stage]];
              ...
              {["#4a4a4a", "#958061", "#a15a3e", "#e3e2dd"].map(
                (colorHex, idx) => (
                  <div
                    ...
                    title={["Black", "Champagne", "Wood", "Silver"][idx]}
